...
function linksSymmetric() {
  for (const a of Object.keys(CITY_LINKS)) {
    for (const b of CITY_LINKS[a]) {
      if (!CITY_LINKS[b] || !CITY_LINKS[b].includes(a)) return false;
    }
  }
  return true;
}
...
async function requestDecisions(cfg, prompt, fetchFn) {
  const doFetch = fetchFn ?? ((url2, init) => fetch(url2, init));
...
console.log("[1] \u9759\u6001\u6570\u636E\u5B8C\u6574\u6027");
assert(linksSymmetric(), "\u57CE\u6C60\u90BB\u63A5\u8868\u5FC5\u987B\u5BF9\u79F0");
{
  let allGenIdsValid = true;
  let allScenarioRefsValid = true;
  for (const sc of SCENARIOS) {
    const fids = new Set(sc.factions.map((f) => f.id));
    const seen = /* @__PURE__ */ new Set();
    for (const c2 of sc.cities) {
      if (!CITY_BY_ID[c2.cityId]) {
        allScenarioRefsValid = false;
        console.error("    \u672A\u77E5\u57CE\u6C60 " + c2.cityId);
      }
      if (!fids.has(c2.owner)) {
        allScenarioRefsValid = false;
        console.error("    \u672A\u77E5\u52BF\u529B " + c2.owner);
      }
      for (const g of c2.generals) {
        if (!GENERALS[g]) {
          allGenIdsValid = false;
          console.error("    \u672A\u77E5\u6B66\u5C06 " + g);
        }
        if (seen.has(g)) {
          allScenarioRefsValid = false;
          console.error("    \u6B66\u5C06\u91CD\u590D\u5206\u914D " + g);
        }
        seen.add(g);
      }
      if (!sc.factions.some((f) => f.ruler && GENERALS[f.ruler])) allScenarioRefsValid = false;
    }
  }
  assert(allGenIdsValid, "\u5267\u672C\u5F15\u7528\u7684\u6B66\u5C06\u5FC5\u987B\u5B58\u5728");
  assert(allScenarioRefsValid, "\u5267\u672C\u5F15\u7528\u7684\u57CE\u6C60/\u52BF\u529B\u5FC5\u987B\u6709\u6548\u4E14\u6B66\u5C06\u4E0D\u91CD\u590D");
  const skillIds = Object.keys(SKILLS);
  let skillOk = true;
  for (const id of Object.keys(GENERALS)) {
    for (const s of skillsFor(GENERALS[Number(id)])) if (!skillIds.includes(s)) skillOk = false;
  }
  assert(skillOk, "skillsFor \u4EA7\u751F\u7684\u6280\u80FD\u5FC5\u987B\u5B58\u5728");
}
ok("\u9759\u6001\u6570\u636E\u68C0\u67E5\u5B8C\u6210");
console.log("[2] \u56DB\u5267\u672C 24 \u4E2A\u6708 AI \u63A8\u6F14");
function checkInvariants(state, tag) {
  const seen = /* @__PURE__ */ new Map();
  let dup = false;
  for (const c2 of state.cities) {
    assert(c2.troops >= 0, `${tag}: ${c2.id} \u5175\u529B\u4E0D\u80FD\u4E3A\u8D1F (${c2.troops})`);
    assert(c2.gold >= 0, `${tag}: ${c2.id} \u91D1\u94B1\u4E0D\u80FD\u4E3A\u8D1F`);
    for (const g of c2.generals) {
      if (seen.has(g)) {
        dup = true;
        console.error(`    \u6B66\u5C06 ${GENERALS[g]?.name} \u540C\u65F6\u4F4D\u4E8E ${seen.get(g)} \u4E0E ${c2.id}`);
      }
      seen.set(g, c2.id);
      assert(!!state.genStates[g], `${tag}: \u5728\u57CE\u6B66\u5C06\u5E94\u6709\u72B6\u6001 (${GENERALS[g]?.name})`);
    }
    for (const cp of c2.captives) {
      if (seen.has(cp.genId)) {
        dup = true;
        console.error(`    \u4FD8\u864F ${GENERALS[cp.genId]?.name} \u540C\u65F6\u4F4D\u4E8E ${seen.get(cp.genId)} \u4E0E ${c2.id}(\u4FD8\u864F)`);
      }
      seen.set(cp.genId, c2.id + "(\u4FD8\u864F)");
      assert(!!state.genStates[cp.genId], `${tag}: \u4FD8\u864F\u5E94\u6709\u72B6\u6001 (${GENERALS[cp.genId]?.name})`);
      assert(cp.from !== c2.owner, `${tag}: \u4FD8\u864F\u4E0D\u5E94\u88AB\u81EA\u5DF1\u52BF\u529B\u7684\u57CE\u770B\u62BC (${GENERALS[cp.genId]?.name})`);
    }
  }
  assert(!dup, `${tag}: \u6B66\u5C06\u4E0D\u80FD\u540C\u65F6\u51FA\u73B0\u5728\u4E24\u5904`);
}
  ...
  for (const f of state.factions.filter((f2) => f2.alive)) {
    ...
    for (const { cmd } of cmds) {
      ...
      if (res.battle) {
        ...
        const result = autoRunBattle(b);
        const captures = applyBattleResult(state, res.battle, result);
        const winFid = result.winner === "atk" ? res.battle.attackerFactionId : res.battle.defenderFactionId;
        if (winFid && captures.length > 0) aiResolveCaptives(state, winFid);
      }
    }
...
for (const sc of SCENARIOS) {
  reseed(12345);
  const playerFid = sc.factions[0].id;
  const state = newGame(sc.id, playerFid);
  checkInvariants(state, sc.id + " \u521D\u59CB");
  let err = null;
  try {
    for (let m = 0; m < 24; m++) runMonth(state);
  } catch (e) {
    err = e;
  }
  assert(err === null, `${sc.name}: 24 \u4E2A\u6708\u63A8\u6F14\u65E0\u5F02\u5E38 ${err ? String(err) : ""}`);
  checkInvariants(state, sc.id + " 24\u6708\u540E");
  const totalCities = state.factions.filter((f) => f.alive).reduce((s, f) => s + factionCities(state, f.id).length, 0);
  assert(totalCities <= state.cities.length, `${sc.name}: \u57CE\u6C60\u5F52\u5C5E\u8BA1\u6570\u5408\u7406`);
  console.log(`  \xB7 ${sc.name}: \u63A8\u6F14\u81F3 ${state.year}\u5E74${state.month}\u6708\uFF0C\u5B58\u6D3B\u52BF\u529B ${state.factions.filter((f) => f.alive).length}/${state.factions.length}`);
}
console.log("[3] \u6218\u6597\u5F15\u64CE");
{
  reseed(777);
  ...
  const weak = [{ genId: null, troops: 1500 }];
  ...
  assert(r.survivors.atk.length > 0, "\u80DC\u65B9\u5E94\u6709\u5E78\u5B58\u8005");
  reseed(42);
  const b2 = createBattle({
    fromCityId: "xiangyang",
    toCityId: "xuchang",
    attackerFactionId: "liubei",
    defenderFactionId: "caocao",
    attackers: [{ genId: 24, troops: 3e3 }],
  ...
  const events = [];
  let usedSkill = false;
  for (let i = 0; i < 12 && !b2.over; i++) {
    const u = currentUnit(b2, events);
    if (!u) break;
    if (u.genId === 24 && u.qi >= 20) {
      const foe = b2.units.find((x) => x.side === "def");
      applyAction(b2, u.uid, { type: "skill", skillId: "huoji", target: foe.uid });
      usedSkill = true;
    } else {
      applyAction(b2, u.uid, aiChooseAction(b2, u));
    }
  }
  assert(usedSkill, "\u6B66\u5C06\u5E94\u80FD\u65BD\u5C55\u8BA1\u7B56");
  reseed(9);
  ...
  const atkUnit = b3.units.find((u) => u.side === "atk");
  applyAction(b3, atkUnit.uid, { type: "retreat" });
  assert(b3.over && b3.winner === "def", "\u653B\u65B9\u5168\u90E8\u64A4\u9000\u540E\u5B88\u65B9\u83B7\u80DC");
  assert(u3 !== null, "\u6218\u6597\u5E94\u80FD\u5F00\u59CB");
}
console.log("[4] \u6307\u4EE4\u6267\u884C");
{
  reseed(5);
  const state = newGame("s194", "liubei");
  const xp = cityById(state, "xiaopei");
  const gold0 = xp.gold;
  const r1 = executeCommand(state, { type: "farm", cityId: "xiaopei" }, "liubei");
  assert(r1.ok && xp.agri > 60 && xp.gold === gold0 - 200, "\u5F00\u57A6\u519C\u4E1A\u751F\u6548");
  const r2 = executeCommand(state, { type: "recruit", cityId: "xiaopei" }, "liubei");
  assert(!r2.ok, "\u5DF2\u884C\u52A8\u7684\u57CE\u4E0D\u80FD\u518D\u6B21\u884C\u52A8");
  const moveRes = executeCommand(state, { type: "move", cityId: "xuchang", to: "luoyang", squads: [{ genId: 1, troops: 1e3 }], gold: 0, food: 0, captives: [] }, "caocao");
  assert(moveRes.ok, "\u8C03\u52A8\u6307\u4EE4\u53EF\u6267\u884C");
  const ly = cityById(state, "luoyang");
  const xc = cityById(state, "xuchang");
  assert(ly.generals.includes(1) && ly.troops === 1e4 && xc.troops === 11e3, "\u8C03\u52A8\u540E\u5175\u529B\u6B66\u5C06\u5230\u4F4D");
  const move2 = executeCommand(state, { type: "move", cityId: "luoyang", to: "xuchang", squads: [{ genId: null, troops: 500 }], gold: 0, food: 0, captives: [] }, "caocao");
  assert(move2.ok && cityById(state, "luoyang").troops === 9500, "\u65E0\u6B66\u5C06\u961F\u4F0D\u4E5F\u53EF\u8C03\u52A8");
}
console.log("[5] \u5E8F\u5217\u5316\u5F80\u8FD4");
{
  const state = newGame("s190", "caocao");
  runMonth(state);
  const round = JSON.parse(JSON.stringify(state));
  assert(JSON.stringify(round) === JSON.stringify(state), "GameState JSON \u5F80\u8FD4\u4E00\u81F4");
}
console.log("[7] \u961F\u4F0D\u7F16\u5236\u3001\u8FD0\u8F93\u4E0E\u4FD8\u864F\u7CFB\u7EDF");
{
  reseed(11);
  const state = newGame("s194", "lvbu");
  const xp = cityById(state, "xiapi");
  const gens = xp.generals.slice(0, 2);
  const capSum = gens.reduce((s, g) => s + troopCapOf(state, g), 0);
  const squads = buildAttackSquads(state, gens, capSum + 3e3);
  const total = squads.reduce((s, x) => s + x.troops, 0);
  assert(total === capSum + MILITIA_CAP, `\u7F16\u961F\u603B\u5175\u529B=\u6B66\u5C06\u4E0A\u9650\u548C+\u6C11\u5175\u4E0A\u9650\uFF08\u5B9E\u9645 ${total}\uFF0C\u671F\u671B ${capSum + MILITIA_CAP}\uFF09`);
  assert(squads.filter((s) => s.genId == null).length <= ATK_MILITIA_MAX, "\u653B\u65B9\u6C11\u5175\u961F\u4E0D\u8D85\u8FC7\u4E0A\u9650");
  let withinCap = true;
  for (const s of squads) if (s.genId != null && s.troops > troopCapOf(state, s.genId)) withinCap = false;
  assert(withinCap, "\u6B66\u5C06\u961F\u4F0D\u4E0D\u8D85\u5E26\u5175\u4E0A\u9650");
  const badAtk = executeCommand(state, {
    type: "attack",
    cityId: "xiapi",
    to: "xiaopei",
    squads: [{ genId: 65, troops: troopCapOf(state, 65) + 500 }]
  }, "lvbu");
  assert(!badAtk.ok, "\u5355\u961F\u8D85\u5E26\u5175\u4E0A\u9650\u7684\u51FA\u5F81\u88AB\u62D2\u7EDD");
  const xc = cityById(state, "xuchang");
  const ly = cityById(state, "luoyang");
  const f0 = xc.food, lyFood0 = ly.food;
  const tr = executeCommand(state, { type: "move", cityId: "xuchang", to: "luoyang", squads: [], gold: 100, food: 800, captives: [] }, "caocao");
  assert(tr.ok && xc.food === f0 - 800 && ly.food === lyFood0 + 800, "\u8C03\u52A8\u91D1\u7CAE\u5230\u8D26");
  const trBad = executeCommand(state, { type: "move", cityId: "xuchang", to: "luoyang", squads: [], gold: 0, food: 0, captives: [] }, "caocao");
  assert(!trBad.ok, "\u7A7A\u8C03\u52A8\u88AB\u62D2\u7EDD");
  const pullOut = (g, fromFid) => {
    const home = state.cities.find((c2) => c2.owner === fromFid && c2.generals.includes(g));
    home.generals.splice(home.generals.indexOf(g), 1);
    cityById(state, "xiapi").captives.push({ genId: g, from: fromFid, tried: false });
  };
  pullOut(2, "caocao");
  state.genStates[2].loyalty = 30;
  let recruited = false;
  for (let i = 0; i < 80 && !recruited; i++) {
    const cap = cityById(state, "xiapi").captives.find((c2) => c2.genId === 2);
    if (cap) cap.tried = false;
    recruited = captiveAction(state, "lvbu", "xiapi", 2, "recruit").ok;
  }
  assert(recruited && cityById(state, "xiapi").generals.includes(2), "\u4F4E\u5FE0\u8BDA\u4FD8\u864F\u6700\u7EC8\u53EF\u88AB\u62DB\u964D");
  assert(!cityById(state, "xiapi").captives.some((c2) => c2.genId === 2), "\u62DB\u964D\u540E\u4E0D\u518D\u5173\u62BC");
  const xpe2 = cityById(state, "xiaopei");
  xpe2.generals.splice(xpe2.generals.indexOf(30), 1);
  ...
  const atk = executeCommand(s2, {
    ...
    cityId: "xiapi",
    to: "xiaopei",
    squads: [{ genId: 65, troops: 3400 }, { genId: 66, troops: 3200 }]
  }, "lvbu");
  assert(atk.ok && !!atk.battle, "\u51FA\u5F81\u6307\u4EE4\u751F\u6210\u6218\u6597");
  if (atk.battle) {
    const defGensBefore = atk.battle.defenders.map((u) => u.genId).filter((g) => g != null);
    const r = autoRunBattle(createBattle(atk.battle));
    applyBattleResult(s2, atk.battle, r);
    for (const g of defGensBefore) {
      const somewhere = s2.cities.some((c2) => c2.generals.includes(g)) || s2.cities.some((c2) => c2.captives.some((cp) => cp.genId === g));
      assert(somewhere || !s2.genStates[g], `\u6218\u8D25\u6B66\u5C06 ${GENERALS[g].name} \u5FC5\u987B\u6709\u53BB\u5411\uFF08\u88AB\u64D2/\u9003\u56DE/\u4E0B\u91CE\uFF09`);
    }
  }
  {
    const s3 = newGame("s194", "lvbu");
    const xc3 = cityById(s3, "xuchang");
    const xp3 = cityById(s3, "xiapi");
    xp3.generals.splice(xp3.generals.indexOf(66), 1);
    xc3.captives.push({ genId: 66, from: "lvbu", tried: false });
    const xpe3 = cityById(s3, "xiaopei");
    xpe3.generals.splice(xpe3.generals.indexOf(30), 1);
    xc3.captives.push({ genId: 30, from: "liubei", tried: false });
    const setup3 = {
      fromCityId: "xiapi",
      toCityId: "xuchang",
    ...
    applyBattleResult(s3, setup3, r3);
...
{
  const d1 = parseDecisions('[{"city":"\u8BB8\u660C","command":"recruit"}]');
  assert(d1.length === 1 && d1[0].city === "\u8BB8\u660C", "\u89E3\u6790\u5E72\u51C0 JSON");
  const d2 = parseDecisions('\u597D\u7684\uFF0C\u51B3\u7B56\u5982\u4E0B\uFF1A\n```json\n[{"city":"\u6D1B\u9633","command":"attack","to":"\u5C0F\u6C9B","generals":["\u5F20\u8FBD"],"troops":5000}]\n```\n\u4EE5\u4E0A\u3002');
  assert(d2.length === 1 && d2[0].command === "attack", "\u89E3\u6790\u5E26\u4EE3\u7801\u5757\u8F93\u51FA");
  let threw = false;
  try {
    parseDecisions("\u6211\u65E0\u6CD5\u51B3\u5B9A");
  } catch {
    threw = true;
  }
  assert(threw, "\u5783\u573E\u8F93\u51FA\u5E94\u629B\u51FA\u5F02\u5E38");
  reseed(3);
  const state = newGame("s207", "caocao");
  const cmds = decisionsToCommands(state, "caocao", [
    { city: "\u6D1B\u9633", command: "recruit" },
    { city: "\u6D1B\u9633", command: "farm" },
    // 重复城池 → 丢弃
    { city: "\u4E0D\u5B58\u5728\u7684\u57CE", command: "farm" },
    // 未知城 → 丢弃
    { city: "\u8BB8\u660C", command: "attack", to: "\u90BA\u57CE", generals: ["\u590F\u4FAF\u60C7"], troops: 1e3 },
    // 打自己 → 丢弃
    { city: "\u957F\u5B89", command: "attack", to: "\u6C49\u4E2D", generals: ["\u66F9\u4EC1"], troops: 999999 }
    // 兵力钳制到城中兵力
  ]);
  ...
  (async () => {
    const cfg = { enabled: true, endpoint: "https://example.com/v1", apiKey: "k", model: "m" };
    const r1 = await planTurnLLM(state, "caocao", cfg, goodFetch);
    assert(r1.usedLLM && r1.commands.length > 0, "LLM \u6B63\u5E38\u65F6\u91C7\u7528 LLM \u51B3\u7B56\uFF08\u542B\u89C4\u5219\u8865\u9F50\uFF09");
    const r2 = await planTurnLLM(state, "caocao", cfg, badFetch);
    assert(!r2.usedLLM && r2.commands.length > 0 && !!r2.error, "LLM \u5931\u8D25\u65F6\u56DE\u9000\u89C4\u5219 AI");
    const prompt = buildPrompt(state, "caocao");
    assert(prompt.includes("\u6D1B\u9633") && prompt.includes("JSON"), "prompt \u5305\u542B\u5C40\u52BF\u4E0E\u683C\u5F0F\u8981\u6C42");
